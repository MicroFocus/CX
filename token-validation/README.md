...
Note that the client ID shows up in the property name as well as the value.  This means you must change the property name if you change the client ID.  For example, to make your client ID be `cxapp` you would use the following settings:

```
com.netiq.cxapp.clientID = cxapp
com.netiq.cxapp.clientPass = secret
com.netiq.cxapp.redirect.url = http://192.168.0.78/oauth.html
```
